#!/usr/bin/env python3
"""Tiny SQLite-backed multi-user account store for WorkLens AI.

Turns the dashboard from one shared password into real logins: many users,
each with their own username + password + role (owner / admin / manager / staff),
grouped under an organisation ("office"). This is the database/SaaS foundation.

Pure standard library (sqlite3 + hashlib) - nothing to pip install.
Passwords are never stored in plain text: we keep a PBKDF2-SHA256 hash + salt.
"""
...
def ensure_admin(db_path, username, password, org_name="My Office"):
    """First-run helper: if there are no users yet, create the org + an OWNER
    account using the given username/password (your old AICAM_PASSWORD).
    Returns True only when it actually created the first account."""
    init_db(db_path)
...
def verify(db_path, username, password):
    """Return a user dict if username+password are correct & active, else None."""
    with _db(db_path) as c:
